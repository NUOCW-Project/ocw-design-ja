...
{if $bn_index != 1}
<a href="index.php?lang={$lang}&mode=pickup&page_type=backnumber&id={$bn_index-1}">&lt;&lt; 前へ</a>
{/if}

{section name="bn_navi" loop=$bn_size}
	{if $smarty.section.bn_navi.iteration != $bn_index}
	<a href="index.php?lang={$lang}&mode=pickup&page_type=backnumber&id={$smarty.section.bn_navi.iteration}">{$smarty.section.bn_navi.iteration}</a>
	{else}<span class="b">{$bn_index}</span>
	{/if}
{/section}

{if $bn_index != $bn_size}
<a href="index.php?lang={$lang}&mode=pickup&page_type=backnumber&id={$bn_index+1}"> &gt;&gt; 次へ</a>
{/if}
</div>
    ...
    <div class="course_contents"><!-- コンテンツセル開始 -->
      ...
      <table class="center_contents" cellspacing="0" summary="PICK UPのタイトルと情報">
        ...
        {strip}
          ...
          {if $bn.relation_id}<a class="pickup_title" href="index.php?mode=course&id={$bn.relation_id}">{/if}
        ...
        {/strip}
          ...
          {if $bn.relation_id}<a href="index.php?mode=course&id={$bn.relation_id}"><img class="pickup_image" src="{$bn.relation_id}/{$bn.file_name}" alt="{$bn.title}" width="150" height="108"></a>
