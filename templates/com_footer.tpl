...
{foreach from=$dept_list key=abbr item=dept name=navigation}
  ...
  {if $abbr!="empty"}
    <li><a href="./{if $lang=='ja'}courselist{elseif $lang=='en'}index{/if}.php?lang={$lang}&amp;mode=l&amp;page_type={$abbr}">{if $lang=='ja'}□{else}&loz;{/if}{$dept.department_nam|regex:"/.*School/":""}</a></li>
  {/if}
