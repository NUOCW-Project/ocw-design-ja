    <div class="footer">
      <img src="./images/2010/img/redline.gif" id="line2" /><!-- 水平線(下) -->
    </div>
  <ul class="navi">
    <li>■ナビゲーション</li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=kyoiku-in">□教養教育院</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=lit">□文学部／文学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=educa">□教育学部／教育発達科学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=nomolog">□法学部／法学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=soec">□経済学部／経済学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=sis">□情報文化学部</a></li>
  </ul>
  
  <ul class="navi">
    <li><br></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=med">□医学部／医学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=eng">□工学部／工学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=agr">□農学部／生命農学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=is">□情報科学研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=gsid">□国際開発研究科</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=math">□多元数理科学研究科</a></li>
  </ul>
  
  <ul class="navi">
    <li><br></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=gcoetop">□グローバルCOE</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=farewell">□退職記念講義</a></li>
    <li><a href="./courselist.php?lang=ja&amp;mode=l&amp;page_type=open">□高校生公開講座</a></li>
  </ul>
  
  <ul class="cont">
    <li>■コンテンツ</li>
    <li>OCWシーズン企画</li>
    <li><a href="./index.php?lang=ja&amp;mode=g&page_type=welcome">ごあいさつ</a></li>
    <li>アクセスランキング</li>
    <li>トピックス</li>
  </ul>
  
  <ul id="link">
    <li><a href="./index.php?lang=ja&amp;mode=g&page_type=about">
	<img src="./images/2010/img/c01aboutnu.gif" id="size_img" alt="名大の授業について"
	     onMouseOver="this.src='./images/2010/img/c01aboutnu_on.gif'" 
	     onMouseOut="this.src='./images/2010/img/c01aboutnu.gif'"/></a></li>
    <li>
      <img src="./images/2010/img/c02answer.gif" id="size_img" alt="用語解説"
	   onMouseOver="this.src='./images/2010/img/c02answer_on.gif'" 
	   onMouseOut="this.src='./images/2010/img/c02answer.gif'"/></li>
    <li><a href="./index.php?lang=ja&amp;mode=g&page_type=vsyllabus">
	<img src="./images/2010/img/c03vs.gif" id="size_img" alt="1分間授業紹介&Podcast"
	     onMouseOver="this.src='./images/2010/img/c03vs_on.gif'" 
	     onMouseOut="this.src='./images/2010/img/c03vs.gif'"/></a></li>
    <li><a href="http://www.media.nagoya-u.ac.jp/sc/">
	<img src="./images/2010/img/c04sc.gif" id="size_img" alt="スタジオチャンネル"
	     onMouseOver="this.src='./images/2010/img/c04sc_on.gif'" 
	     onMouseOut="this.src='./images/2010/img/c04sc.gif'"/></a></li>
    <li>
      <img src="./images/2010/img/c05link.gif" id="size_img" alt="リンク"
	   onMouseOver="this.src='./images/2010/img/c05link_on.gif'" 
	   onMouseOut="this.src='./images/2010/img/c05link.gif'"/></li>
  </ul>
  
  <div id="proviso">
    <hr />
    <img src="./images/2010/img/nu.gif" class="nu" />
    <p class="nu">名大の授業Webサイト、およびこのサイトで公開されている講義資料は、<br>
      「名大の授業について」の記載条件のもとで利用することができます（仮）</p>
  </div>
</div><!--"#contents" end-->

<!-- 右端URL画像部分 -->
<div class="main_back_url">
</div>
<!-- 右上の人物 -->
<div class="main_back_adachi">
</div>

</div><!-- "main_back" end -->
</div><!--"main" end-->
</body>
</html>
