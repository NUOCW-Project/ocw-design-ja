<a href="./index.php?lang=ja&mode=g&page_type=about"><img src="../img/top/menu01.png" onMouseOver="this.src='../img/top/menu01_on.png'" onMouseout="this.src='../img/top/menu01.png'" class="menu0" /></a>
<a href="./index.php?lang=ja&mode=g&page_type=legal"><img src="../img/top/menu02.png" onMouseOver="this.src='../img/top/menu02_on.png'" onMouseout="this.src='../img/top/menu02.png'" class="menu0" /></a>
<a href="./index.php?lang=ja&mode=g&page_type=vsyllabus"><img src="../img/top/menu03.png" onMouseOver="this.src='../img/top/menu03_on.png'" onMouseout="this.src='../img/top/menu03.png'" class="menu0" /></a>
<div class="menu0"><a href="http://www.media.nagoya-u.ac.jp/sc/"><img src="../img/top/menu04.png" onMouseOver="this.src='../img/top/menu04_on.png'" onMouseout="this.src='../img/top/menu04.png'" /></a><p id="date"><b>2009. 10. 04UP</b></p></div>

<div id="sp_feature">
  <img src="../img/top/t_special.png" />
  <img src="../img/top/backno.png" class="jump" />
  <br />
  <img src="../img/redline.gif" class="line" />
  <br />
  <img src="../img/d_sp_feature.JPG" /><!-- 画像「季節の講義　メッセージ」ダミー -->
</div> <!-- "sp_feature" end -->
    
<div id="greeting">
  <b>ごあいさつ<br />
  <img src="../img/d_yamamoto.JPG" id="yamamoto" />
  <p>名古屋大学<br />オープンコースウェア委員長<br />山本一良</p></b>
     ここでは普段名大生しか見ることのできない講義資料をたくさん公開しています。<br />
      <a href="./index.php?lang=ja&mode=g&page_type=welcome" class="jump">more...</a><!-- 後でリンクを張る -->
</div><!-- "greeting" end -->

<div class="waku">
<div class="topics"><!-- ボックス「アクセスランキング」 -->
  <img src="../img/top/t_ranking.png" class="subheading" />
  <br />
  <img src="../img/top/dot.png" />
  <dl id="ranking">
  <dt>順位 講義名</dt>
      <dd>教員名</dd>
   <dt class="pri-tri_l">１<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">海外実地研修</a></dt>
      <dd class="pri-tri_t">前田敬一郎教授</dd>
   <dt class="secon_l">２<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">フィンランド語</a></dt>
      <dd class="secon_t">山里敬也准教授</dd>
   <dt class="pri-tri_l">３<a href="./index.php?lang=ja&mode=c&id=106&page_type=index">環境行動論</a></dt>
      <dd class="pri-tri_t">佐久間淳一准教授</dd>
   <dt>４<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">メディア論</a></dt>
      <dd>佐久間淳一准教授</dd>
   <dt>５<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">通信技術</a></dt>
      <dd>山里敬也准教授</dd>
   <dt>６<a href="./index.php?lang=ja&mode=c&id=106&page_type=index">メディア論</a></dt>
      <dd>廣瀬幸雄教授</dd>
   <dt>７<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">通信技術</a></dt>
      <dd>山里敬也准教授</dd>
   <dt>８<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">通信技術</a></dt>
      <dd>後藤明史准教授</dd>
   <dt>９<a href="./index.php?lang=ja&mode=c&id=106&page_type=index">メディア論</a></dt>
      <dd>廣瀬幸雄教授</dd>
   <dt>10<a href="./index.php?lang=ja&mode=c&id=38&page_type=index">メディア論</a></dt>
      <dd>山里敬也准教授</dd>
  </dl>
</div><!-- 「アクセスランキング」end  -->
</div>

<div class="waku">
<div class="topics"><!-- ボックス「お知らせ」-->
  <img src="../img/top/t_news.png" class="subheading" />
  <img src="../img/top/backno.png" class="jump" />
  <br />
  <img src="../img/top/dot.png" />
    <dl class="notice">
     <dt>-2009/12/24</dt>
     <dd><a href="./index.php?lang=ja&mode=c&id=106&page_type=index">数理科学展望I【高橋雅英教授】を公開しました。</a></dd>
    <dt>-2009/12/24</dt>
    <dd><a href="./index.php?lang=ja&mode=c&id=129&page_type=index">グローバルCOE系統講義(医学部)【高橋雅英教授】を公開しました。</a></dd>
    <dt>-2009/12/24</dt>
    <dd><a href="./index.php?lang=ja&mode=c&id=38&page_type=index">グローバルCOE系統講義(医学部)【高橋雅英教授】を公開しました。</a></dd>
    </dl>
</div><!-- 「お知らせ」end  -->
</div>

<div class="waku">
<div class="topics"><!-- ボックス「プレスリリース」-->
  <img src="../img/top/t_release.png" class="subheading" />
      <img src="../img/top/backno.png" class="jump" />
  <br />
  <img src="../img/top/dot.png" />
    <dl class="notice">
     <dt>-2009/12/24</dt>
    <dd><span>新デザインになりました。</span><a href="./index.php?lang=ja&mode=c&id=129&page_type=index">情報リテラシー【横井茂樹教授】を公開しました。</a></dd>
    </dl>
</div><!-- 「プレスリリース」end -->
</div>
