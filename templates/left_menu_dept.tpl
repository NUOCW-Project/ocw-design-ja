...
<!-- 各学部ページへのリンクリスト -->
{foreach from=$dept_list item="loop"}
  {* "m01kyoiku-in"などの値を持つ変数を作る *}
  {assign var="basename" value="md_`$loop.department_abbr`"}
  {if $lang == "ja"}
    {assign var="dept_name" value=$loop.department_name}
  {else}
    {assign var="dept_name" value=$loop.department_name_e}
  {/if}

{strip}
  {* その学部表示時にはボタン画像dummy.gifからチェックマーク付き(hoge_h.gif)に差し替え *}
  <li class="m_{$loop.department_abbr}">
  {if $curr_department_abbr == $loop.department_abbr}
  <img src="./images/{$lang}/{$basename}_h.gif" alt="{$dept_name}">
  {else}<a href="index.php?lang={$lang}&mode=l&page_type={$loop.department_abbr}"><img src="./images/common/dummy.gif" alt="{$dept_name}"></a>
  {/if}
  </li>
{/strip}
