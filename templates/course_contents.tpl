...
<div id="caption">
  ...
  <div id="data"><!-- 背景灰色のボックス -->
    ...
    {if $course_info.f_date_ja}<br><b>日時</b>：<div id="class_for">{$course_info.f_date_ja}{$course_info.f_time_ja}</div>{/if}
    {if $course_info.f_place_ja}<br><b>場所</b>：<div id="class_for">{$course_info.f_place_ja}</div>{/if}
  </div>
</div><!--"caption" end-->
{/if}    
